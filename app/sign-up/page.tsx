...
export default function SignUpPage() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Try logging in first (if they already have an account with this number)
    ...
    <div className="relative min-h-screen bg-slate-50 dark:bg-[#030014] text-slate-900 dark:text-[#f8fafc] flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      <div className="w-full max-w-md z-10">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-sm text-slate-505 dark:text-slate-400 mt-2">Get started with your integrated AI personal assistant</p>
        </div>
        ...
        <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-8 backdrop-blur-md shadow-xl dark:shadow-2xl relative">
          ...
          {authMethod === 'phone' && (
            <>
              {!otpSent ? (
                /* Step 1: Input Name, Phone & Choose Method */
                <form onSubmit={handleSendOtp} className="space-y-5">
                  <div>
                    <label htmlFor="phoneName" className="block text-xs font-semibold text-slate-650 dark:text-slate-300 uppercase tracking-wider mb-2">
                      Display Name
                    </label>
                    <div className="relative">
                      <input
                  ...
                  {/* Channel Choice */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-655 dark:text-slate-300 uppercase tracking-wider mb-3">
