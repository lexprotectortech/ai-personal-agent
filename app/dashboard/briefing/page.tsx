...
export default function BriefingPage() {
  ...
  const [formCategories, setFormCategories] = useState<string[]>(['email', 'messages', 'tasks']);
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const dailyBrief = data.history?.find((r: BriefingRun) => !r.categories === false || r.name.toLowerCase().includes('daily'));
        if (dailyBrief) {
  ...
  const handleRunManual = async (scheduleId: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        alert('Briefing generation has been queued! Refresh in a few seconds to see the new run.');
        fetchHistory();
      } else {
  ...
  const handleDeleteSchedule = async (scheduleId: string) => {
    if (!user || !confirm('Are you sure you want to delete this custom briefing schedule?')) return;
  ...
  const handleToggleApp = (app: string) => {
    setFormApps(prev => prev.includes(app) ? prev.filter(a => a !== app) : [...prev, app]);
  };
  ...
  const getCategoryCountAndSummary = (categoryKey: string) => {
    ...
    switch (categoryKey) {
      case 'email':
        return { count: 3, summary: 'Sarah Jenkins requested dashboard layout changes' };
      case 'messages':
        return { count: 4, summary: 'Marketing Campaign proposals and devnet upgrades' };
      case 'mentions':
        return { count: 1, summary: 'Tagged in Dockerfile staging approval issue' };
      case 'tasks':
        return { count: 2, summary: 'Finalize contracts and resolve calendar conflict' };
      case 'followups':
        return { count: 1, summary: 'Daniel requested review on layout design feedback' };
      default:
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto px-4 md:px-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-200 dark:border-white/5 pb-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 max-w-xl">
            View executive summaries from all linked communication accounts. Schedule background briefings or run them on demand.
          </p>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <h3 className="text-2xl font-bold font-display text-slate-950 dark:text-white">
                {latestDailyBrief?.topBrief?.title || 'Sarah Jenkins requested dashboard adjustments'}
              </h3>
              <p className="text-slate-600 dark:text-slate-350 text-sm md:text-base max-w-3xl leading-relaxed">
                {latestDailyBrief?.topBrief?.summary || 'Sarah requested card layout updates to support HSL theme colors. WhatsApp and Telegram messages indicate a marketing campaign draft is due and devnet upgrade is scheduled.'}
              </p>
            </div>
            <div className="flex items-center space-x-2 text-xs text-slate-400 dark:text-slate-500 font-medium">
              <Clock className="w-3.5 h-3.5 mr-1" />
              Updated {latestDailyBrief?.created_at ? new Date(latestDailyBrief.created_at).toLocaleTimeString() : 'Just now'}
            </div>
          </div>
          <button
            onClick={() => router.push(`/dashboard/briefing/details?id=${latestDailyBrief?.id || 'daily'}`)}
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold font-display text-slate-900 dark:text-white">Grouped Updates</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { key: 'email', name: 'Email Inbox', desc: 'Mail notifications', icon: Mail, borderHover: 'hover:border-red-500/30 bg-red-500/5 text-red-500' },
            { key: 'messages', name: 'Messages & Chats', desc: 'WhatsApp & Telegram', icon: MessageSquare, borderHover: 'hover:border-emerald-500/30 bg-emerald-500/5 text-emerald-500' },
            { key: 'mentions', name: 'Mentions', desc: 'Github & Dev forums', icon: AtSign, borderHover: 'hover:border-blue-500/30 bg-blue-500/5 text-blue-500' },
            { key: 'tasks', name: 'Action Tasks', desc: 'Calendar & Project logs', icon: ListTodo, borderHover: 'hover:border-indigo-500/30 bg-indigo-500/5 text-indigo-500' },
            { key: 'followups', name: 'Follow-ups Pending', desc: 'Outstanding replies', icon: Clock, borderHover: 'hover:border-amber-500/30 bg-amber-500/5 text-amber-500' }
          ].map((cat) => {
            ...
            return (
              <div
                key={cat.key}
                onClick={() => router.push(`/dashboard/briefing/details?id=${latestDailyBrief?.id || 'daily'}&category=${cat.key}`)}
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center border border-current bg-transparent`}>
                      <Icon className="w-5 h-5" />
                    </div>
                    <div>
                      <span className="text-sm font-bold text-slate-800 dark:text-white block">{cat.name}</span>
                      <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider font-semibold block">{cat.desc}</span>
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/5 rounded-2xl p-6 shadow-sm dark:shadow-none">
          <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-white/5 mb-4">
            <h3 className="text-lg font-bold font-display text-slate-900 dark:text-white">Custom briefing Schedules</h3>
            <span className="text-xs text-slate-400 dark:text-slate-500 font-semibold">{schedules.length} Active Rules</span>
          </div>
              ...
              {schedules.map((sched) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <h4 className="text-sm font-bold text-slate-800 dark:text-white">{sched.name}</h4>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{sched.description || 'No description provided.'}</p>
                    </div>
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${
                      sched.priority_level === 'High' ? 'bg-rose-500/10 border-rose-500/20 text-rose-500' :
                      sched.priority_level === 'Medium' ? 'bg-amber-500/10 border-amber-500/20 text-amber-500' :
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {sched.selected_apps?.map(app => (
                      <span key={app} className="inline-flex items-center space-x-1 text-[10px] px-2 py-0.5 rounded bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 text-slate-600 dark:text-slate-350">
                        {getSourceIcon(app)}
        ...
        <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/5 rounded-2xl p-6 shadow-sm dark:shadow-none">
          <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-white/5 mb-4">
            <h3 className="text-lg font-bold font-display text-slate-900 dark:text-white">Recent Briefing Runs</h3>
            <span className="text-xs text-slate-400 dark:text-slate-500 font-semibold">{runs.length} Runs</span>
          </div>
          ...
          {loadingHistory ? (
            ...
            <div className="space-y-4 max-h-[460px] overflow-y-auto pr-1">
              {runs.map((run) => (
                <div
                  key={run.id}
                  onClick={() => router.push(`/dashboard/briefing/details?id=${run.id}`)}
                ...
                >
                  <div className="space-y-2 min-w-0 pr-4">
                    <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                      <span className="text-xs font-bold text-slate-800 dark:text-white capitalize truncate">{run.name}</span>
                      <span className="text-[9px] font-semibold text-slate-400 dark:text-slate-500">
                        {new Date(run.created_at).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-2">
                      {run.topBrief?.summary || 'Briefing analysis completed successfully.'}
                    </p>
                  </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          {/* Backdrop blur */}
          <div className="absolute inset-0 bg-slate-950/40 dark:bg-slate-950/60 backdrop-blur-sm" onClick={() => setIsModalOpen(false)} />
          ...
          <div className="bg-white dark:bg-[#0c0a21] border border-slate-250 dark:border-white/10 rounded-3xl w-full max-w-lg p-6 relative z-10 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/5 pb-3">
              <h3 className="text-lg font-bold font-display text-slate-900 dark:text-white">Configure Custom Briefing</h3>
              <button onClick={() => setIsModalOpen(false)} className="p-1 hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg transition-colors cursor-pointer text-slate-400">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateSchedule} className="space-y-4">
              {/* Name */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Briefing Name</label>
                <input
                  type="text"
                  required
              ...
              {/* Goal Description */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Description or Goal</label>
                <textarea
              ...
              {/* Selected Apps */}
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide block">Selected Apps</label>
              ...
              {/* Selected Categories */}
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide block">Selected Categories</label>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { id: 'email', name: 'Email' },
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide block">Scheduled Time</label>
                  <input
                    type="time"
                    required
                    value={formTime}
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide block">Frequency</label>
                  <select
                    value={formFrequency}
              ...
              {/* Priority */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide block">Priority Level</label>
                <select
                  value={formPriority}
              ...
              <div className="flex items-center justify-end space-x-3 pt-4 border-t border-slate-100 dark:border-white/5">
                ...
                >
                  {actionLoading === 'create' && <Loader2 className="w-3.5 h-3.5 mr-2 animate-spin" />}
                  Save Schedule
                </button>
              </div>
            </form>
