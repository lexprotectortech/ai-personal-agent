...
Ref: Gmail WhatsApp Telegram`
  ...
  const injectPlatformMarkdown = (text: string) => {
    const platforms = ['Gmail', 'WhatsApp', 'Slack', 'Outlook', 'Discord', 'LinkedIn', 'Telegram'];
    let processed = text;
  ...
  const promptSuggestions = [
    ...
    },
    {
      ...
      prompt: "Check my Gmail notifications and draft a reply to Sarah Jenkins' email feedback",
      ...
      <div className="flex-1 flex flex-col border border-slate-200 dark:border-white/5 rounded-3xl bg-white dark:bg-slate-950/20 shadow-sm dark:shadow-none backdrop-blur-sm overflow-hidden">
        ...
        <div className="flex-1 p-8 overflow-y-auto space-y-6 flex flex-col bg-slate-50/10 dark:bg-slate-950/5">
            ...
            <div className="space-y-4 mb-4 animate-fade-in pl-11">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {promptSuggestions.map((item, idx) => (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${item.iconBg} flex-shrink-0`}>
                      {item.icon}
                    </div>
          ...
          {messages.map((msg, index) => {
            ...
            const displayContent = isUser ? msg.content : injectPlatformMarkdown(msg.content);
              ...
              <div key={index} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} space-y-1`}>
                <div className={`flex items-start space-x-3 max-w-[85%] ${isUser ? 'flex-row-reverse space-x-reverse' : ''}`}>
                  ...
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                    ...
                    {isUser ? <span className="text-xs font-bold uppercase">{userName.substring(0, 2)}</span> : <Bot className="w-4 h-4" />}
                  </div>

                  {/* Message bubble */}
                  <div className={`p-4 rounded-2xl shadow-sm dark:shadow-none leading-relaxed border ${
                    isUser
                    ...
                    {isUser ? (
                      <p className="text-sm font-semibold whitespace-pre-wrap">{displayContent}</p>
                    ) : (
                      <div className="prose dark:prose-invert max-w-none text-sm font-medium">
                        <ReactMarkdown
                          remarkPlugins={[remarkGfm]}
                          components={{
                            h1: ({ children }) => <h1 className="text-base font-bold mt-4 mb-2 text-slate-800 dark:text-white font-display border-b border-slate-200/50 dark:border-white/5 pb-1">{children}</h1>,
                            h2: ({ children }) => <h2 className="text-sm font-bold mt-3 mb-1.5 text-slate-800 dark:text-white font-display">{children}</h2>,
                            h3: ({ children }) => <h3 className="text-xs font-bold mt-2.5 mb-1 text-slate-800 dark:text-white font-display">{children}</h3>,
                            p: ({ children }) => <p className="mb-2 last:mb-0 leading-relaxed text-slate-700 dark:text-slate-300">{children}</p>,
                            ul: ({ children }) => <ul className="list-disc pl-5 mb-2.5 space-y-1 text-slate-700 dark:text-slate-300">{children}</ul>,
                            ol: ({ children }) => <ol className="list-decimal pl-5 mb-2.5 space-y-1 text-slate-700 dark:text-slate-300">{children}</ol>,
                            li: ({ children }) => <li className="text-xs sm:text-sm leading-relaxed">{children}</li>,
                            a: ({ href, children }) => <a href={href} target="_blank" rel="noopener noreferrer" className="text-indigo-600 dark:text-indigo-400 hover:underline font-semibold flex items-center inline-flex space-x-1">{children} <ExternalLink className="w-3 h-3 ml-0.5 inline" /></a>,
                            code: ({ inline, className, children, ...props }: any) => {
                              ...
                              return !inline ? (
                                ...
                                <code className="bg-slate-100 dark:bg-slate-900 text-purple-650 dark:text-purple-400 px-1.5 py-0.5 rounded text-xs font-mono" {...props}>
                                  {children}
                                </code>
                              );
                            },
                            ...
                            img: ({ src, alt }) => {
                              ...
                              if (['gmail', 'whatsapp', 'slack', 'outlook', 'discord', 'linkedin', 'telegram'].includes(platform)) {
                                return (
                              ...
                              return <img src={src} alt={alt} className="rounded-xl my-2 max-w-full" />;
                            },
                            table: ({ children }) => <div className="overflow-x-auto my-3.5 border border-slate-200 dark:border-white/5 rounded-xl shadow-inner"><table className="min-w-full divide-y divide-slate-200 dark:divide-white/5 text-xs text-left">{children}</table></div>,
                            thead: ({ children }) => <thead className="bg-slate-50 dark:bg-white/5 font-semibold text-slate-750 dark:text-slate-350">{children}</thead>,
                            tbody: ({ children }) => <tbody className="divide-y divide-slate-250 dark:divide-white/5 bg-white dark:bg-slate-950/20">{children}</tbody>,
                            tr: ({ children }) => <tr className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors">{children}</tr>,
                            th: ({ children }) => <th className="px-4.5 py-2">{children}</th>,
                            td: ({ children }) => <td className="px-4.5 py-2 font-medium text-slate-650 dark:text-slate-400">{children}</td>,
                          }}
                        >
                          {displayContent}
                        </ReactMarkdown>
          ...
          {isThinking && (
            <div className="flex items-start space-x-3 max-w-[85%] animate-pulse">
              ...
              <div className="p-4 rounded-2xl rounded-tl-none border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/5 text-sm text-slate-600 dark:text-slate-400 flex items-center space-x-2 font-semibold shadow-sm">
                ...
                <span>OmniSync Agent searching connected integrations and formulating response...</span>
              </div>
            </div>
          )}
