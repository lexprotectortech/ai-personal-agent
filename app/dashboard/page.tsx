...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const priorityCached = localStorage.getItem(`dashboard_priority_${user.id}`);
    if (priorityCached) {
      try {
  ...
  const fetchSummaryCard = async (force = false) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        localStorage.setItem(`dashboard_summary_${user.id}`, JSON.stringify(data));
      } else {
  ...
  const fetchPriorityCard = async (force = false) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        localStorage.setItem(`dashboard_priority_${user.id}`, JSON.stringify(data));
      } else {
  ...
  const handleCommandSubmit = (e: React.FormEvent) => {
    ...
    if (commandQuery.trim()) {
      router.push(`/dashboard/ai-agent?q=${encodeURIComponent(commandQuery.trim())}`);
    }
  };
    ...
    <div className="space-y-6 max-w-6xl mx-auto relative pb-24">
      ...
      <div className="flex items-start justify-between">
        ...
        >
          ...
          <span className="hidden sm:inline">{loadingBrief ? 'Syncing...' : 'Refresh'}</span>
        </button>
      </div>
        ...
        <div className="bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/10 rounded-2xl shadow-sm dark:shadow-none flex flex-col min-h-[340px]">
          ...
          <div className="flex items-center justify-between px-5 pt-5 pb-3">
            <div className="flex items-center space-x-2.5">
              ...
              <h3 className="text-sm font-bold text-slate-800 dark:text-white tracking-wide">Today&apos;s Brief</h3>
            </div>
            <button
              onClick={() => router.push('/dashboard/briefing')}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 pointer-events-none">
        <div className="max-w-6xl mx-auto px-8 pb-5 pointer-events-auto">
          <form
            onSubmit={(e) => {
              e.preventDefault();
              const input = (e.target as HTMLFormElement).elements.namedItem('bottomChat') as HTMLInputElement;
              if (input?.value.trim()) {
