...
export default function AlertsPage() {
  ...
  const [summaryData, setSummaryData] = useState<{ summary: string; nextAction: string; draft: string } | null>(null);
  ...
  const [formNotification, setFormNotification] = useState<'In-App' | 'Email' | 'WhatsApp'>('In-App');
  const [formFrequency, setFormFrequency] = useState<'Real-time' | 'Hourly' | 'Daily'>('Real-time');
  ...
  useEffect(() => {
    ...
    const setupRealtime = async () => {
      ...
      try {
        ...
        const subscribeRes = await insforge.realtime.subscribe(`alerts:${user.id}`);
        ...
        if (subscribeRes.ok) {
          ...
          insforge.realtime.on('alert_triggered', (newAlert: TriggeredAlert) => {
            ...
            showToast(`New ${newAlert.priority_level} Priority Alert: ${newAlert.title}`, 'warning');
          });
        }
      } catch (err) {
  ...
  const handleCheckNow = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        showToast(`Alert Check Completed! Found ${data.triggeredCount} new alerts.`, 'success');
        loadData();
      } else {
  ...
  // Toggle Rule Status
  const handleToggleRuleStatus = async (ruleId: string, currentActive: boolean) => {
    try {
  ...
  const handleSnooze = async (alertId: string, minutes: number) => {
    ...
    try {
      ...
      if (error) {
        ...
        setTriggeredAlerts((prev) =>
          prev.map((a) => (a.id === alertId ? { ...a, status: 'Snoozed', snoozed_until: snoozedUntil } : a))
        );
  ...
  const handleRegenerateReply = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/ai/compose', {
        ...
        body: JSON.stringify({
          sender: selectedAlert.title.split(']')[1]?.trim() || selectedAlert.title || 'User',
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      let recipient = selectedAlert.title.split(']')[1]?.split(' ')[0] || 'mock-recipient';
      ...
      if (data.success) {
        ...
        showToast(`Reply sent successfully via ${selectedAlert.source_app}!`, 'success');
  ...
  const resolvedAlertsCount = triggeredAlerts.filter((a) => a.status === 'Resolved').length;

  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      {/* Toast Alert banner */}
      {toastMessage && (
        <div className={`fixed bottom-6 right-6 z-50 p-4 rounded-xl border shadow-2xl flex items-center space-x-3 backdrop-blur-md animate-in slide-in-from-bottom duration-300 ${
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold font-display text-slate-800 dark:text-white tracking-wide">Alert Center</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Configure smart conditions and monitor updates across connected messaging & calendar feeds.
          </p>
        </div>
        <div className="flex items-center space-x-3 flex-wrap">
          ...
          >
            ...
            <span>{checkingAlerts ? 'Checking Feeds...' : 'Check Feeds Now'}</span>
          </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass-panel rounded-2xl p-5 border border-slate-200 dark:border-white/5 shadow-sm relative overflow-hidden">
          ...
          <div className="flex items-center space-x-3">
            ...
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Active Rules</span>
          </div>
          <p className="text-2xl font-bold font-display text-slate-800 dark:text-white mt-3">{activeAlertRulesCount}</p>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-5 border border-slate-200 dark:border-white/5 shadow-sm relative overflow-hidden">
          ...
          <div className="flex items-center space-x-3">
            ...
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Triggered Today</span>
          </div>
          <p className="text-2xl font-bold font-display text-slate-800 dark:text-white mt-3">{triggeredTodayCount}</p>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-5 border border-slate-200 dark:border-white/5 shadow-sm relative overflow-hidden">
          ...
          <div className="flex items-center space-x-3">
            ...
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">High Priority</span>
          </div>
          <p className="text-2xl font-bold font-display text-slate-800 dark:text-white mt-3">{highPriorityAlertsCount}</p>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-5 border border-slate-200 dark:border-white/5 shadow-sm relative overflow-hidden">
          ...
          <div className="flex items-center space-x-3">
            ...
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Resolved Alerts</span>
          </div>
          <p className="text-2xl font-bold font-display text-slate-800 dark:text-white mt-3">{resolvedAlertsCount}</p>
        </div>
      </div>
          ...
          <div className="glass-panel border border-slate-200 dark:border-white/5 rounded-2xl p-6 shadow-sm">
            ...
            {loadingAlerts ? (
              <div className="py-20 flex flex-col items-center justify-center space-y-3">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400">Loading alert feeds...</span>
              </div>
              ...
              <div className="py-20 flex flex-col items-center justify-center text-center space-y-4">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">All Quiet</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">No rules have triggered recently. Set up a rule or check feeds now.</p>
                </div>
              </div>
            ) : (
              <div className="mt-6 relative pl-6 border-l border-slate-200 dark:border-white/5 space-y-8">
                {triggeredAlerts.map((alert) => {
                  const logo = PLATFORM_LIST.find((p) => p.id === alert.source_app)?.logo || '/default-platform.svg';
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`absolute -left-[31px] top-5 w-2 h-2 rounded-full border border-white dark:border-[#030014] ${
                      ...
                      <div className="flex items-center space-x-3">
                        <span className={`text-[9px] uppercase font-bold tracking-wider px-2 py-0.5 rounded hidden sm:inline ${
                          alert.status === 'Resolved'
          ...
          <div className="glass-panel border border-slate-200 dark:border-white/5 rounded-2xl p-6 shadow-sm relative overflow-hidden">
            ...
            {loadingSuggestions ? (
              <div className="py-12 flex flex-col items-center justify-center space-y-2">
                ...
                <span className="text-[10px] text-slate-500 dark:text-slate-400">Scanning activity logs...</span>
              </div>
            ) : aiSuggestions.length === 0 ? (
              <div className="py-8 text-center text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                Connect more accounts or refresh feeds to unlock automated suggestions.
              </div>
            ) : (
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-white/5">
                      <div className="flex items-center space-x-1.5">
                        {sug.selected_apps.map((app) => {
                          const appLogo = PLATFORM_LIST.find((p) => p.id === app)?.logo || '/default-platform.svg';
                          return (
                            <Image
                              key={app}
                              src={appLogo}
          ...
          <div className="glass-panel border border-slate-200 dark:border-white/5 rounded-2xl p-6 shadow-sm">
            ...
            {loadingRules ? (
              <div className="py-12 flex flex-col items-center justify-center space-y-2">
                ...
                <span className="text-[10px] text-slate-500 dark:text-slate-400">Loading rules...</span>
              </div>
            ) : alertRules.length === 0 ? (
              <div className="py-8 text-center text-xs text-slate-500 dark:text-slate-400">
                No custom alert rules configured yet. Click "Create New Alert" to add one.
              </div>
            ) : (
                ...
                {alertRules.map((rule) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-slate-800 dark:text-white truncate max-w-[150px]">{rule.name}</span>
                    ...
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 line-clamp-1">{rule.description || 'No description'}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/85 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#0b081e] border border-slate-200 dark:border-white/10 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden relative max-h-[90vh] flex flex-col">
            ...
            <div className="p-6 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-950/40 flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white font-display tracking-wide">Create New Alert Rule</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">Establish a trigger condition evaluated continuously across messages.</p>
              </div>
            </div>

            {/* Form Body */}
            <form onSubmit={handleCreateRule} className="p-6 overflow-y-auto space-y-4 flex-1">
              {/* Rule Name */}
              <div className="space-y-1">
                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400">Alert Rule Name</label>
                <input
                  type="text"
                  required
              ...
              {/* Description */}
              <div className="space-y-1">
                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400">Description</label>
                <input
                  type="text"
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 block">Monitor Selected Apps</label>
                  ...
                  {PLATFORM_LIST.map((platform) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          if (isChecked) {
                            setFormApps(formApps.filter((a) => a !== platform.id));
                          } else {
                      ...
                      >
                        <Image src={platform.logo} alt={platform.name} width={14} height={14} className="object-contain" />
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400">Alert Trigger Condition</label>
                <textarea
                  required
                  rows={2}
              ...
              <div className="grid grid-cols-3 gap-3">
                {/* Priority */}
                <div className="space-y-1">
                  <label className="text-[9px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 block">Priority</label>
                  <select
                    value={formPriority}
                ...
                {/* Method */}
                <div className="space-y-1">
                  <label className="text-[9px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 block">Notify Via</label>
                  <select
                ...
                {/* Frequency */}
                <div className="space-y-1">
                  <label className="text-[9px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 block">Frequency</label>
                  <select
                    value={formFrequency}
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400">Perform Action on Trigger</label>
                <input
                  type="text"
              ...
              <div className="pt-4 flex items-center justify-end space-x-3 border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-950/20 -mx-6 -mb-6 p-4">
                ...
                >
                  {submittingRule && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
                  <span>{submittingRule ? 'Saving...' : 'Save Alert Rule'}</span>
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-white dark:bg-[#0b081e] border border-slate-200 dark:border-white/10 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden relative max-h-[92vh] flex flex-col">
            ...
            <div className="p-6 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-950/40 flex items-center space-x-4">
              <div className="w-12 h-12 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 flex items-center justify-center p-2 flex-shrink-0">
                <Image
                  src={PLATFORM_LIST.find((p) => p.id === selectedAlert.source_app)?.logo || '/default-platform.svg'}
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1">
              ...
              <div className="space-y-1.5">
                <h4 className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Alert Trigger Content</h4>
              ...
              {loadingSummary ? (
                <div className="py-12 border border-dashed border-slate-200 dark:border-white/5 rounded-xl bg-slate-50/30 dark:bg-slate-950/10 flex flex-col items-center justify-center space-y-2">
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400">Synthesizing alert details and action plan...</span>
                </div>
              ) : (
                    ...
                    <div className="space-y-3 border-t border-slate-200 dark:border-white/5 pt-4">
                      ...
                      <div className="flex items-center justify-between pt-1">
                        <div className="flex items-center space-x-2 text-[10px] text-slate-400 dark:text-slate-500">
                          ...
                          <span>Sends reply from connected app account JID/address</span>
                        </div>
                        <button
                          onClick={handleSendReply}
                          disabled={sendingReply || sendingReplySuccess || !replyDraft}
                        ...
                        >
                          ...
                          <span>
                            {sendingReply ? 'Sending Reply...' : sendingReplySuccess ? 'Reply Dispatched!' : 'Send Reply Now'}
                          </span>
                        </button>
                      </div>
                    </div>
            ...
            <div className="p-4 border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-950/40 flex flex-wrap gap-3 items-center justify-between">
              {/* Snooze Dropdown */}
              <div className="flex items-center space-x-2">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Snooze:</span>
              ...
              <div className="flex items-center space-x-3">
                ...
                >
                  ...
                  <span>{selectedAlert.status === 'Resolved' ? 'Resolved' : 'Resolve Alert'}</span>
                </button>
              </div>
            </div>
          </div>
