		...
		tools: [
			{
				...
				desc: "List recent email threads in the user's inbox.",
			},
			{
				name: "gmail_get_thread",
				desc: "Retrieve detailed messages and headers for a specific email thread.",
			},
			{
				name: "gmail_search_emails",
				desc: "Find messages matching query tags (e.g., from, subject, dates).",
			},
			{
				name: "gmail_create_draft",
				desc: "Auto-create a draft reply to an email thread with AI suggestions.",
			},
			{
				name: "gmail_send_email",
				desc: "Deliver an email directly from the authenticated inbox.",
			},
			{
				name: "gmail_apply_label",
				desc: "Add or remove custom labels from email threads.",
			},
			{
				name: "gmail_list_labels",
				desc: "List all custom and system labels in the account.",
			},
		],
	},
	{
		...
		tools: [
			{
				name: "wa_fetch_recent_messages",
				desc: "Fetch recent messages across all active chats.",
			},
			{
				name: "wa_read_chat_history",
				desc: "Read detailed chat history for a specific contact or group JID.",
			},
			{
				name: "wa_send_message",
				desc: "Send a text message to a specific contact JID.",
			},
			{ name: "wa_search_chats", desc: "Search chats by contact name or JID." },
			{
				name: "wa_summarize_conversations",
				desc: "Summarize the recent conversation history with a contact or group using AI.",
			},
			{
				name: "wa_get_contact_details",
				desc: "Retrieve profile details and metadata for a specific contact JID.",
			},
			{ name: "wa_list_groups", desc: "List all joined WhatsApp group chats." },
			{
				name: "wa_fetch_group_messages",
				desc: "Fetch recent messages specifically from a group chat room.",
			},
			{
				name: "wa_send_group_messages",
				desc: "Send a message to a WhatsApp group chat room.",
			},
		],
	},
	{
		...
		tools: [
			{
				...
				desc: "List active channels in the authorized Slack workspace.",
			},
			{
				name: "slack_post_message",
				desc: "Post automated AI digests or tasks directly to a text channel.",
			},
		],
	},
	{
		...
		tools: [
			{
				...
				desc: "Retrieve scheduled calendar events for roadmap auditing.",
			},
			{
				name: "outlook_resolve_conflicts",
				desc: "Identify and resolve overlapping calendar sessions.",
			},
		],
	},
	{
		...
		tools: [
			{ name: "discord_list_guilds", desc: "List linked Discord servers." },
			{
				...
				desc: "Publish task digests to a text channel.",
			},
		],
	},
	{
		...
		tools: [
			{
				...
				desc: "List profile connection metadata.",
			},
			{
				name: "linkedin_post_update",
				desc: "Share digests as updates on the feed.",
			},
		],
	},
	{
		...
		tools: [
			{
				name: "telegram_list_updates",
				desc: "Fetch incoming messages sent to the bot.",
			},
			{
				name: "telegram_send_alert",
				desc: "Send instant briefs or summaries directly to a Telegram conversation.",
			},
		],
	},
	{
		...
		tools: [
			{
				name: "webhook_ingest_payload",
				desc: "Ingest and parse inbound raw JSON webhook streams.",
			},
			{
				name: "webhook_trigger_dispatch",
				desc: "Dispatch outgoing data payloads to registered webhooks.",
			},
		],
	},
];

const ACTION_PARAMS_CONFIG: Record<
	string,
...
> = {
	...
	wa_read_chat_history: [
		{
			label: "Recipient JID or Phone Number",
			key: "jid",
			placeholder: "e.g. 919876543210 or 12345-67890@s.whatsapp.net",
			type: "text",
		},
		{
			label: "Limit (number of messages)",
			key: "limit",
			placeholder: "e.g. 20",
			type: "number",
		},
	...
	wa_search_chats: [
		{
			label: "Search Query",
			key: "query",
			placeholder: "e.g. John or Budget",
			type: "text",
		},
	],
	wa_summarize_conversations: [
		{
			label: "Chat JID or Phone Number",
			key: "jid",
			placeholder: "e.g. 919876543210 or group-id@g.us",
			type: "text",
		},
	],
	wa_get_contact_details: [
		{
			label: "Contact JID or Phone Number",
			key: "jid",
			placeholder: "e.g. 919876543210",
			type: "text",
		},
	...
	wa_fetch_group_messages: [
		{
			label: "Group JID",
			key: "jid",
			placeholder: "e.g. 12036302492109@g.us",
			type: "text",
		},
...
function IntegrationsContent() {
	...
	// Dialog state
	const [settingsPlatform, setSettingsPlatform] = useState<Platform | null>(
		null,
	);
	...
	const [whatsappConnectingStatus, setWhatsappConnectingStatus] = useState<
		...
		const checkRedirectCallback = async () => {
			...
			if (code) {
				...
				try {
					...
					setError(
						err.message ||
							"Failed to synchronize Gmail connection state after authorization.",
					);
				} finally {
			...
			if (connectedParam) {
				...
				try {
					...
					setError(
						"Failed to synchronize connection state after authorization.",
					);
				} finally {
	...
	const handleConnectWhatsApp = async (e: React.FormEvent) => {
		...
		try {
			...
			console.error(err);
			setWhatsappError(
				"Failed to generate linking code. Please check your network and try again.",
			);
			setWhatsappConnectingStatus("error");
		}
	};
	...
	const handleConnectSimulatedWhatsApp = async () => {
		...
		try {
			...
			console.error(err);
			setWhatsappError(
				"Failed to enable simulated mode. Please check your connection.",
			);
			setWhatsappConnectingStatus("error");
		}
	};
			...
			<div className="bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 rounded-2xl p-6 backdrop-blur-sm relative overflow-hidden">
				...
				<div className="flex items-start space-x-4">
					...
					<div className="space-y-1">
						<h2 className="text-xl font-bold font-display text-slate-900 dark:text-white tracking-wide">
							Connected Integrations
						</h2>
						<p className="text-sm text-slate-500 dark:text-slate-400">
							Link communication platforms to your personal AI agent. Connection
							states are synchronized directly to your secure InsForge database.
						</p>
					</div>
				</div>
			</div>
						...
						<div
							...
							<div className="flex flex-col items-center text-center space-y-4 mb-4">
								...
								<div>
									...
									<span
										className={`inline-flex items-center text-[10px] font-bold uppercase tracking-wider mt-1.5 px-2 py-0.5 rounded-full ${
											isConnected ?
										...
										{isConnected ?
											platform.id === "whatsapp" && isWhatsappSimulated ?
												"Connected (Simulated)"
											:	"Connected"
										:	"Disconnected"}
									</span>
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
					<div className="bg-white dark:bg-[#0b081e] border border-slate-200 dark:border-white/10 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden relative max-h-[90vh] flex flex-col">
						...
						<div className="p-6 border-b border-slate-200 dark:border-white/5 flex items-center space-x-4 bg-slate-50 dark:bg-slate-950/40">
							...
							<div>
								<h3 className="text-lg font-bold text-slate-900 dark:text-white font-display tracking-wide">
									Connect WhatsApp
								</h3>
								<p className="text-xs text-slate-500 dark:text-slate-400">
									Link your account using a pairing code
								</p>
							</div>
						</div>
								...
								<form onSubmit={handleConnectWhatsApp} className="space-y-4">
									<div className="space-y-2">
										...
										<span className="text-[10px] text-slate-400 dark:text-slate-500 leading-normal block">
											Enter the number linked to your WhatsApp mobile app.
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
					<div className="bg-white dark:bg-[#0b081e] border border-slate-200 dark:border-white/10 w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden relative max-h-[85vh] flex flex-col">
						...
						<div className="p-6 border-b border-slate-200 dark:border-white/5 flex items-center space-x-4 bg-slate-50 dark:bg-slate-950/40">
							...
							<div>
								<h3 className="text-lg font-bold text-slate-900 dark:text-white font-display tracking-wide">
									{settingsPlatform.name} MCP Configuration{" "}
									{settingsPlatform.id === "whatsapp" &&
										isWhatsappSimulated &&
										"(Simulated)"}
								</h3>
								<p className="text-xs text-slate-500 dark:text-slate-400">
									Available model context protocol tools and actions
								</p>
							</div>
						</div>
						...
						<div className="p-6 overflow-y-auto space-y-4 flex-1">
							<h4 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-widest">
							...
							<div className="space-y-3">
								{settingsPlatform.tools.map((tool) => (
									<div
										key={tool.name}
										className="p-4 rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-950/20 space-y-2">
							...
							{settingsPlatform.id === "whatsapp" && (
								<div className="mt-8 border-t border-slate-200 dark:border-white/5 pt-6 space-y-4">
									...
									<p className="text-xs text-slate-500 dark:text-slate-400">
										Test and execute any WhatsApp MCP tools live against your
										connected session.
									</p>
									...
									{isWhatsappSimulated && (
										<div className="p-3 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-600 dark:text-indigo-400 text-xs flex items-center space-x-2">
											...
											<span>
												Simulated Mode is active. Tool executions will return
									...
									{ACTION_PARAMS_CONFIG[selectedAction] &&
										ACTION_PARAMS_CONFIG[selectedAction].length > 0 && (
									...
									{actionResult && (
										<div className="space-y-2">
											...
											{(
												selectedAction === "wa_summarize_conversations" &&
		...
		<Suspense
			fallback={
				<div className="flex flex-col items-center justify-center h-96 space-y-4">
					<Loader2 className="w-8 h-8 text-indigo-500 animate-spin" />
					<span className="text-sm text-slate-400">
						Loading integrations interface...
					</span>
				</div>
			}>
			<IntegrationsContent />
