...
export const checkSchedulesCron = schedules.task({
  ...
  run: async (payload) => {
    console.log(`[Cron] Checking for due briefing runs at: ${payload.timestamp}`);
    ...
    const { data: dueSchedules, error } = await insforge.database
      .rpc("claim_due_briefing_schedules", { lookahead_minutes: 15 });
    ...
    console.log(`[Cron] Found ${dueSchedules.length} schedules due. Triggering generator runs...`);
    ...
    const runPromises = dueSchedules.map(async (schedule: any) => {
      try {
        ...
        console.log(`[Cron] Triggered generateBriefing for schedule: ${schedule.id} (${schedule.name})`);
      } catch (err: any) {
        console.error(`[Cron] Failed to trigger run for schedule ${schedule.id}:`, err);
      }
    });
...
export const generateBriefing = task({
  ...
  run: async (payload: {
    ...
    const { scheduleId, userId, name, description, selectedApps, selectedCategories, priorityLevel } = payload;
    ...
    if (filteredNotifications.length === 0) {
      ...
      const { data, error } = await insforge.database.rpc("save_generated_briefing", {
        user_id_val: userId,
    ...
    if (!briefData) {
      ...
      const counts = {
        email: filteredNotifications.filter(n => n.category === "email").length,
        messages: filteredNotifications.filter(n => n.category === "messages").length,
        mentions: filteredNotifications.filter(n => n.category === "mentions").length,
        tasks: filteredNotifications.filter(n => n.category === "tasks").length,
        followups: filteredNotifications.filter(n => n.category === "followups").length,
      };

      briefData = {
        topHighlightedBrief: {
          title: "Simulated Dashboard Summary",
          summary: "Sarah Jenkins requested urgent UI alignment feedback. Node upgrade scheduled for Solana devnet."
        },
        categorySummaries: {
          email: { count: counts.email, summary: counts.email > 0 ? "You have new unread emails from Sarah Jenkins." : "No new emails." },
          messages: { count: counts.messages, summary: counts.messages > 0 ? "New messages on WhatsApp and Telegram." : "No new messages." },
          mentions: { count: counts.mentions, summary: counts.mentions > 0 ? "You were tagged in a Github issue." : "No new mentions." },
          tasks: { count: counts.tasks, summary: counts.tasks > 0 ? "Immediate roadmap alignment items pending." : "No outstanding tasks." },
          followups: { count: counts.followups, summary: counts.followups > 0 ? "Daniel is waiting for your design critique feedback." : "No pending follow-ups." }
        },
        items: filteredNotifications.map((n, idx) => ({
          ...
          timestamp: "Just now",
          actionRequired: true,
          suggestedAction: n.category === "email" ? "Reply to Sarah" : (n.category === "tasks" ? "Resolve conflict" : "Acknowledge message")
        }))
      };
    }

    // Save briefing to DB using the RPC
    const { data: savedId, error: saveError } = await insforge.database.rpc("save_generated_briefing", {
      user_id_val: userId,
      schedule_id_val: scheduleId || null,
      name_val: name,
    ...
    if (saveError) {
      console.error("[Worker] Error saving generated briefing to DB:", saveError);
      throw saveError;
    }
...
export const checkAlertsCron = schedules.task({
  ...
  run: async (payload) => {
    console.log(`[Cron] Checking alert rules for all users at: ${payload.timestamp}`);
    ...
    const triggerPromises = users.map(async (user: any) => {
      try {
        ...
        console.error(`[Cron] Failed to trigger alert check for user ${user.id}:`, err);
      }
    });
...
export const checkUserAlertRules = task({
  ...
  run: async (payload: { userId: string }) => {
    ...
    try {
      ...
      console.log(`[Worker] Alert check complete. Triggered ${newAlerts.length} new alerts for user ${userId}.`);
      return { success: true, triggeredCount: newAlerts.length };
    } catch (err: any) {
      console.error(`[Worker] Error running evaluateAlertRules for user ${userId}:`, err);
      throw err;
    }
  },
});
